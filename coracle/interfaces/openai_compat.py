...
* :func:`list_models_response` -- builds the OpenAI ``/v1/models`` list
  payload, with the default ``coracle`` entry first.
...
def list_models_response(*, now: Callable[[], float] = time.time) -> dict[str, object]:
    ...
    The default ``coracle`` profile is always first in ``data``.
    """
...
def submit_job(
    ...
    * If ``force_class`` is ``None`` (the default ``coracle`` profile),
