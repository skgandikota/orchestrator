...
This module is the API-only entry point used by the coracle's ``deep`` and
``research`` pipelines. Providers + models are configured in
``coracle/config/settings.toml`` under ``[providers.*]``; environment
...
def _resolve_settings_path(path: Path | None) -> Path:
    if path is not None:
        return Path(path)
    env = os.environ.get("CORACLE_SETTINGS")
    if env:
        return Path(env)
