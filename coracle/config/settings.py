...
class FsToolSettings(BaseModel):
    workspace_root: str = str(Path.home() / "coracle-workspace")
...
class WebToolSettings(BaseModel):
    """Settings for the web fetch + search tool (`coracle.tools.web`)."""

    user_agent: str = "coracle-bot/0.1 (+https://github.com/skgandikota/coracle)"
    allow_private: bool = False
    brave_api_key: str | None = None


class StateSettings(BaseModel):
    """Settings for the durable state store (``coracle.core.state``)."""

    db_path: Path = Field(default=Path("./coracle.db"))
...
def _apply_env_overrides(
    ...
    """Override nested keys from env vars shaped ``CORACLE__SECTION__KEY``."""
