...
coracle as a drop-in model provider.

Endpoints
---------
* ``GET  /v1/models``           - lists the coracle model profiles.
...
class PipelineEvent:
    """One event in the coracle pipeline event stream.
...
def _auth_dependency(request: Request) -> None:
    expected = os.environ.get("CORACLE_API_TOKEN")
    if not expected:
        return
