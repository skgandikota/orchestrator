"""Tool registry for the coracle's coder model.
...
* The minimal in-process registry that ``coracle.tools.mcp_client``
...
class Registry:
    ...
    def register(self, tool: Tool, *, replace: bool = False) -> None:
        ...
        ``coracle.tools.mcp_client`` before the remote schema arrives).
        """
        with self._lock:
    ...
    def dispatch(self, name: str, args: dict[str, Any]) -> ToolResult:
        ...
        by :mod:`coracle.tools.mcp_client` instead.
        """
        start = perf_counter()
        with self._lock:
        ...
        if inspect.isawaitable(result):
            return ToolResult(
                ok=False,
                error=(
                    f"tool {name!r} is async; use the async dispatcher (coracle.tools.mcp_client)"
                ),
                duration_ms=_ms(),
            )
