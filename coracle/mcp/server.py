"""MCP stdio server exposing the coracle job manager.
...
:class:`coracle.api.tasks.JobManager` so the MCP and HTTP
...
from coracle.api.tasks import JobManager, get_job_manager
...
TOOL_DEFINITIONS: list[mt.Tool] = [
    mt.Tool(
        name="submit_job",
        description="Submit a new coracle job; returns the job_id immediately.",
        inputSchema={
            "type": "object",
            "properties": {
                "user_msg": {
                ...
                "model": {
                    "type": ["string", "null"],
                    "description": "Optional model id; defaults to the coracle default.",
                },
            },
            "required": ["user_msg"],
...
def build_server(mgr: JobManager | None = None) -> Server:
    ...
    mgr:
        ...
        returned by :func:`coracle.api.tasks.get_job_manager`.
    """
